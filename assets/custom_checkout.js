     ...
     jQuery(document).ready(function(){
        ...
        //alert('hii');
       jQuery(".kaktusc-cart__checkout").hide();
        var unifyCartData = "https://e-commercedemo.myshopify.com/cart.json";
          
        jQuery.getJSON(unifyCartData, function(jsonData) {
            ...
            jQuery('.kaktusc-cart__checkout-wrap').append(html);
            jQuery('#shopi_all_data').val(dt);
        });
       
      });

     jQuery(document).on('click','.cart-prod-qnt',function(event) {
    ...
    //  setTimeout(function(){
    //jQuery(".wallet-button-fade-in").children().children().remove();
    ...
    //      html+="</form>";
    //     jQuery(".wallet-button-fade-in").children().append(html);
    //        jQuery('#shopi_all_data').val(dt);
    //     });
    //    },1000);
